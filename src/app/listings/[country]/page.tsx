...
export default async function CountryListingsPage({ params }: Props) {
    ...
    return (
        <main className="min-h-screen bg-[#FAF9F6] px-6 py-12">
            <div className="max-w-6xl mx-auto">
                <div className="mb-8">
                    <Link href="/listings" className="text-sm text-[#718096] hover:text-[#2F5233] transition-colors mb-4 inline-block">
                        ← Back to all areas
                    </Link>
                    <h1 className="text-4xl font-semibold text-[#2D3748] capitalize">
                        Homes in {countryQuery}
                    </h1>
                </div>
                ...
                {listings.length === 0 && (
                    <div className="text-center py-20">
                        ...
                        <p className="text-[#718096]">We couldn&apos;t find any vacant properties in {countryQuery}.</p>
                    </div>
                )}
            </div>
        </main>
